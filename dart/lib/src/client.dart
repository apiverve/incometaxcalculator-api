...
class IncometaxcalculatorValidationException implements Exception {
  final List<String> errors;

  IncometaxcalculatorValidationException(this.errors);

  @override
  String toString() => 'IncometaxcalculatorValidationException: ${errors.join("; ")}';
}
...
/// Income Tax Calculator API client.
///
/// For more information, visit: https://apiverve.com/marketplace/incometaxcalculator?utm_source=dart&utm_medium=readme
///
/// Parameters:
/// * [income] (required) - Gross annual income in USD [min: 0]
/// * [filingStatus] (required) - Filing status
/// * [year] - The tax year to use for brackets. Defaults to the previous year. [min: 2000, max: 2030]
class IncometaxcalculatorClient {
  final String apiKey;
  ...
  static final Map<String, ValidationRule> _validationRules = <String, ValidationRule>{
    'income': ValidationRule(type: 'number', required: true, min: 0),
    'filing_status': ValidationRule(type: 'string', required: true),
    'year': ValidationRule(type: 'integer', required: false, min: 2000, max: 2030),
  };

  IncometaxcalculatorClient(this.apiKey, {
    this.baseUrl = 'https://api.apiverve.com/v1/incometaxcalculator',
    http.Client? httpClient,
  }) : _httpClient = httpClient ?? http.Client();

  /// Validates parameters against defined rules.
  /// Throws [IncometaxcalculatorValidationException] if validation fails.
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw IncometaxcalculatorValidationException(errors);
    }
  }

  /// Execute a request to the Income Tax Calculator API.
  ///
  /// Parameters are validated before sending the request.
  Future<IncometaxcalculatorResponse> execute(Map<String, dynamic> params) async {
    // Validate parameters
    _validateParams(params);
    if (apiKey.isEmpty) {
      throw IncometaxcalculatorException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return IncometaxcalculatorResponse.fromJson(json);
      } else if (response.statusCode == 401) {
        throw IncometaxcalculatorException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw IncometaxcalculatorException('Resource not found');
      } else {
        throw IncometaxcalculatorException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is IncometaxcalculatorException) rethrow;
      throw IncometaxcalculatorException('Request failed: $e');
    }
  }
...
/// Exception thrown by the Income Tax Calculator API client.
class IncometaxcalculatorException implements Exception {
  final String message;

  IncometaxcalculatorException(this.message);

  @override
  String toString() => 'IncometaxcalculatorException: $message';
}
