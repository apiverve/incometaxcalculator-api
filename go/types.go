// Package incometaxcalculator provides a Go client for the Income Tax Calculator API.
//
// For more information, visit: https://apiverve.com/marketplace/incometaxcalculator?utm_source=go&utm_medium=readme
...
// Request contains the parameters for the Income Tax Calculator API.
//
// Parameters:
//   - income (required): number - Gross annual income in USD [min: 0]
//   - filing_status (required): string - Filing status
//   - year: integer - The tax year to use for brackets. Defaults to the previous year. [min: 2000, max: 2030]
type Request struct {
	Income float64 `json:"income"` // Required
	FilingStatus string `json:"filing_status"` // Required
...
func (r *Request) Validate() error {
	rules := map[string]ValidationRule{
		"income": {Type: "number", Required: true, Min: float64Ptr(0)},
...
// ResponseData contains the data returned by the Income Tax Calculator API.
type ResponseData struct {
	...
	IncomeAfterTax float64 `json:"incomeAfterTax"`
	Brackets []BracketsItem `json:"brackets"`
}

// BracketsItem represents an item in the Brackets array.
type BracketsItem struct {
	Rate float64 `json:"rate"`
	RatePercent string `json:"ratePercent"`
	RangeMin int `json:"rangeMin"`
	RangeMax int `json:"rangeMax"`
	TaxableAmount int `json:"taxableAmount"`
