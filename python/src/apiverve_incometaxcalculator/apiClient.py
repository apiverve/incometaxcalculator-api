...
class IncometaxcalculatorAPIClientError(Exception):
...
class ValidationError(IncometaxcalculatorAPIClientError):
...
class IncometaxcalculatorAPIClient:
    # Validation rules for parameters (generated from schema)
    VALIDATION_RULES = {"income": {"type": "number", "required": True, "min": 0}, "filing_status": {"type": "string", "required": True}, "year": {"type": "integer", "required": False, "min": 2000, "max": 2030}}
    ...
    def __init__(self, api_key: str, secure: bool = True, debug: bool = False):
        """
        Initialize the IncometaxcalculatorAPIClient with the API key and optional settings

        :param api_key: Your APIVerve API key
        :param secure: Deprecated. Always set to True
        :param debug: Enable debug logging (default: False)
        :raises IncometaxcalculatorAPIClientError: If API key is invalid
        """
        ...
        self.secure = secure
        self.debug = debug
        self.base_url = 'https://api.apiverve.com/v1/incometaxcalculator'
        self.headers = {
    ...
    def _validate_api_key(self, api_key: str) -> None:
        ...
        if not api_key or not api_key.strip():
            raise IncometaxcalculatorAPIClientError(
    ...
    def _log(self, message: str) -> None:
        ...
        """
        if self.debug:
            print(f"[IncometaxcalculatorAPIClient DEBUG] {message}")
    ...
    def execute(self, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:  # Returns IncomeTaxCalculatorData in 'data' field
        """
        Execute the API request

        :param params: API parameters (optional)
        :return: API response as a dictionary
        :raises IncometaxcalculatorAPIClientError: If the request fails or returns an error
        ...
        try:
            ...
            raise IncometaxcalculatorAPIClientError(f"Request timeout: {str(e)}")
        except requests.exceptions.ConnectionError as e:
            self._log(f"Connection error: {str(e)}")
            raise IncometaxcalculatorAPIClientError(f"Connection error: {str(e)}")
        except requests.exceptions.RequestException as e:
            self._log(f"Request exception: {str(e)}")
            raise IncometaxcalculatorAPIClientError(f"Request failed: {str(e)}")
